 ...
 * Changelog:
 * 1.3.7
 *  - now supports basic coloring (e.g: console.log('%cMy message, white on black!', 'color: #FFF; background-color: #000;'); )
 *
        ...
        try {
          ...
          if (isRepeat(message, method) && method.indexOf('time') === -1) {
            ...
            if (typeof message === 'object' && method !== 'assert' && method !== 'timeEnd') {
              ...
              //handle colors
              var parts = message.split('%c'); parts.shift();
              var l = parts.length;
